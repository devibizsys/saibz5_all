 ...
 *
 */
public class PPASDialectImpl extends DBDialectImpl
{
	private static final Log log = LogFactory.getLog(PPASDialectImpl.class);
	...
	public String getPagingSQL(String strSQL, int nStartPos, int nPageSize, String strMajor, String strMajorDirection, String strMinor, String strMinorDirection)
	{
		...
		if (!StringHelper.isNullOrEmpty(strMajor))
		{
			...
			{
				script.append("SELECT * FROM (Select m1.*, row_number()  as SRFROWINDEX from ( select * from (%1$s) pagetemp ORDER BY pagetemp.%2$s %3$s) m1  ", strSQL, strMajor, strMajorDirection);
			}
			else
			{
				script.append("SELECT * FROM (Select m1.*, row_number()  as SRFROWINDEX from ( select * from (%1$s) pagetemp ORDER BY pagetemp.%2$s %3$s,pagetemp.%4$s %5$s) m1 ", strSQL, strMajor, strMajorDirection, strMinor,strMinorDirection);
			}
	...
	public DBCallResult callSql(java.sql.Connection connection, String strCommand, SqlParamList list, int nTimeOut)
	{
		...
		try
		{
			...
			PPASDataSetImpl dataSet = new PPASDataSetImpl(null, cstmt);
